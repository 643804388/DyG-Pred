...
# 训练集准备
# for i in os.listdir("data/new/train"):
#     data_path = os.path.join("data/new/train", i)
#     train = pickle.load(open(data_path, "rb"))
#     train_list.append(train[-1])
# train_data = pickle.load(open("data/new/train0.data", "rb"))
# train_data = [train_data[1], train_data[100], train_data[400], ]
# train_data = train_list
# 分数据集
# sample = random.sample(train_list, 40)
# un_sample = [i for i in train_list if i not in sample]
# train_data = un_sample
# test_data = sample
# 测试集准备
# for i in os.listdir("data/new/test"):
#     data_path = os.path.join("data/new/test", i)
...
optimizer = torch.optim.Adam(sgnn.parameters(), lr=3e-5, weight_decay=1e-5)
...
for epoch in range(0, 200):
    if train == 1:
        ...
        if epoch >= 25 and epoch % 5 == 0:
            torch.save(sgnn.state_dict(), 'models/output/example_model11{}.pth'.format(epoch))
