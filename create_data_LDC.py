...
for i, data in enumerate(graph_dataset):
    # train
    if i in [389, 441, 463, 464, 465, 466, 476, 594]:
        continue
    # if i >= 200:
    #     continue
    # test
    ...
    for event in events_list:
        events = event['name'].split(" ")
        if events[0] == '<arg1>':
            events_split.append(events[1])
        else:
            events_split.append(events[0])
    j = 0
    for eff_path in eff_path_list:
        ...
        result = []
        for a in eff_path:
            result.append(events_split[a])
        # result = [events_split for i in eff_path]
        # train_list.append(result)
        # test_list.append(result)
        if 0 <= len(eff_path_list) % 10 < 3:
            test_list.append(result)
        # if 2 <= len(eff_path_list) % 10 < 4:
...
with open('./data/new/{}/{}nodeLDC{}.json'.format('test', 'test', graph_i), 'w') as file:
    json.dump(test_list, file)

with open('./data/new/{}/{}nodeLDC{}.json'.format('train', 'train', graph_i), 'w') as file:
    json.dump(train_list, file)

with open('./data/new/{}/{}nodeLDC{}.json'.format('valid', 'valid', graph_i), 'w') as file:
