...
class SGNN(nn.Module):
    ...
    def __init__(self, sch_hidden_size, ske_hidden_size):
        ...
        self.GCN = GraphConvolutionalNetwork(128,128,128)
    def set_l(self, l):
    ...
    def forward(self, s, batch_size, nodes_sch, A_sch, nodes_ske, A_ske, S_adj, T_adj):
        ...
        # 数据放入cuda
        nodes_sch_embedding, A_sch, sch_adj = nodes_sch_embedding.to(self.device), A_sch.to(self.device), sch_adj.to(
            self.device)
        ...
        for i in range(1, nodes_sch_embedding.shape[1]):
            ...
            node_id, similarity_matrix_sub, nodes_sch_S_T, entry_loss, accuracy_gen = self.encoder(i, similarity_matrix_sub, A_sch,
                                                                                     nodes_sch_embedding, sch_adj,
                                                                                     T_adj, S_adj, nodes_sch_S_T,
                                                                                     idx_list)
        ...
        graph_matrix = torch.where(similarity_matrix_min_max_normalization > 0.5, torch.tensor(1).to(self.device),
        ...
        S_count = torch.tensor([1 * b, 2 * b, 2 * b, 2 * b, 2 * b, 2 * b, 2 * b, 2 * b, 2 * b, 1 * b],
        ...
        nodes_S_T = self.S_conv(similarity_matrix_sub, nodes_S_T)

        # 获取融合图
        S_T_Graph_nodes = torch.concat([nodes_S_T, nodes_Static], dim=-1)
        S_T_Graph_nodes = self.reduce(S_T_Graph_nodes)
        S_T_Graph_nodes = self.reduce_norm(S_T_Graph_nodes)
        # 融合图与动态图作交叉注意力
...
def symmetric_normalized_laplacian(adjacency_matrix):
    ...
    # 计算对称归一化拉普拉斯矩阵
    laplacian_matrix = torch.eye(adjacency_matrix.size(0)) - \
                       torch.mm(torch.mm(torch.inverse(torch.sqrt(degree_matrix)), adjacency_matrix),
...
class GraphConvolutionLayer(nn.Module):
    ...
    def forward(self, x, adjacency_matrix):
        """
        x: 输入特征，形状为 (batch_size, num_nodes, input_dim)
        adjacency_matrix: 图的邻接矩阵，形状为 (batch_size, num_nodes, num_nodes)
        """
        # 通过邻接矩阵进行特征聚合
        aggregated = torch.bmm(adjacency_matrix, x)  # 批次矩阵乘法
        # 使用线性层进行线性变换
...
class GraphConvolutionalNetwork(nn.Module):
    ...
    def forward(self, x, adjacency_matrix, laplacian_matrix):
        """
        x: 输入特征，形状为 (batch_size, num_nodes, input_dim)
        adjacency_matrix: 图的邻接矩阵，形状为 (batch_size, num_nodes, num_nodes)
        laplacian_matrix: 图的拉普拉斯矩阵，形状为 (batch_size, num_nodes, num_nodes)
        """
        # 第一层图卷积
        x = self.conv1(x, adjacency_matrix)
        # 使用拉普拉斯算子
        x = torch.bmm(laplacian_matrix, x)  # 批次矩阵乘法
