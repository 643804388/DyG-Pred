...
class myBert(nn.Module):
    ...
    def __init__(self):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(r"./bert-large-uncased")
        self.model = BertModel.from_pretrained(r"./bert-large-uncased").to(self.device)
...
class myBert_model(nn.Module):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def __init__(self):
        super(myBert_model, self).__init__()
        # self.tokenizer = BertTokenizer.from_pretrained(r"F:\Plot_Reasoning\bert-base-chinese")
